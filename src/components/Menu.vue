<template 
<header class="p-3 text-bg-dark">
<header>
    <div class="container">
      <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start  ">
        <a href="/" class="d-flex align-items-center mb-2 mb-lg-0 text-white text-decoration-none">
          <svg class="bi me-2" width="40" height="32" role="img" aria-label="Bootstrap"><use xlink:href="#bootstrap"></use></svg>
        </a>
       
        <ul class="nav col-12 col-lg-auto me-lg-auto mb-2 justify-content-center mb-md-0 ">
          <li><a href="/" class="nav-link px-2 text-secondary">Home</a></li>
          <li><a href="/product" class="nav-link px-2 text-secondary">Product</a></li>
          <li><a href="/contact" class="nav-link px-2 text-secondary">Contact</a></li>

          <li><a href="/about" class="nav-link px-2 text-secondary">About</a></li>
          <li><a href="#" class="nav-link px-2 text-secondary">Add Product</a></li>
        </ul>

        <form class="col-12 col-lg-auto mb-3 mb-lg-0 me-lg-3" role="search">
          <input type="search" class="btn btn-warning" placeholder="Search..." aria-label="Search">
        </form>

        <div class="text-end">
          <a href = "/login"> <button  class="btn btn-warning me-2">Login</button> </a>
         <a href = "/sing-up"> <button class="btn btn-warning">Sign-up</button></a>
        </div>
      </div>
    </div>
</header>

</template>


<script>
export default {
    name: 'Menu'
}
</script>


 

    
